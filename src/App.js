...
import BtnListAdd from './components/BtnListAdd/BtnListAdd.jsx'
...
function App() {
  ...
  useEffect(() => {
    axios
      .get('https://61af2ce03e2aba0017c4909e.mockapi.io/lists')
      .then(({ data }) => {
        console.log(data)
      })
    axios
  ...
  const onEditTask = (listId, taskObj) => {
    const newTaskText = window.prompt('Переиминовать задачу', taskObj.text)
    console.log(taskObj)
    if (!newTaskText) {
      return
    }
